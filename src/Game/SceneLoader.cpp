...
SceneNode *SceneLoader::parseEntity(Scene *scene, const std::string &entityJson)
{
    try
    {
        ...
        {
            char *tag = new char[entity["tag"].get<std::string>().length() + 1];
            std::strcpy(tag, entity["tag"].get<std::string>().c_str());
            node->setTag(tag);
        }

        if (entity.contains("children") && entity["children"].is_array())
        {
            for (const auto &childJson : entity["children"])
            {
                SceneNode *childNode = parseEntity(scene, childJson.dump());
                if (childNode)
                {
                    scene->attach(childNode, node);
                }
            }
        }
